...
def main():
    ...
    # ======================
    sess_dir = str(get_sess_dir(args, analysis_cfg))
    ...
    # Fit Detectors
    # =============
    data_cols = ['com_to', 'detector_type', 'network', 'drive', 'sense', 'num_tones', 'bath_temp', 'coldload_temp', 'tone_placement_method']
    ...
    # ========================
    dashboard_data = ['detector_type', 'network', 'drive', 'sense', 'LNA_bias']#, 'coldload_temp', 'bath_temp']
    network_iterator = tqdm(networks, desc = 'Plotting Networks', colour='red')   
    for network in network_iterator:     
        det_type, network_num = network.data.select('detector_type', 'network').to_numpy()[0]
        network_iterator.set_postfix_str(f'Network: {det_type} {network_num}')

        #plot_heatmap('drive', 'sense', 'frac_mismatch_rotate_origin_shift_origin_rotate_unwind_rotate_f_white_noise', network, sess_dir, logz=True, include=args.tones, over=['LNA_bias'], xlabel='Drive [dB]', ylabel='Sense [dB]', clabel=r'$\sqrt{S_{xx}}\ \left[Hz^{-1/2}\right]$')
        ...
        #plot_heatmap('drive', 'sense', 'freq_diss_ratio', network, sess_dir, include=args.tones, over=['LNA_bias'], xlabel='Drive [dB]', ylabel='Sense [dB]', clabel='Frequency/Dissipation Ratio')
        ...
        for det, *data in tqdm(network.data.select(['detector'] + dashboard_data).iter_rows(), desc = 'Plotting Detectors', total=network.data.height, colour='orange'):
            det.stream.mag()
            det.stream.phase()
            det.IQ_unwind(prefix='nonlinear_fit', data='targ', delay_col='nonlinear_fit_delay_ns', recalc=False)
            det.IQ_circle_real(prefix='unwind_rotate_nonlinear_fit', loc='origin', data='targ', recalc=False)
            det.IQ_circle_rotate(prefix='origin_shift_origin_rotate_unwind_rotate_nonlinear_fit', data='targ', rotation='mismatch', recalc=False)
            det.targ.phase(prefix='mismatch_rotate_origin_shift_origin_rotate_unwind_rotate_nonlinear_fit')
...
def filter_network(network):
    ...
    network.data = network.data.filter(pl.col('LNA_bias') == 8).filter(pl.col('drive') == 16).filter(pl.col('sense') == 5)
...
def plot_det_dashboard(det, data_cols, sess_dir, viz_cfg, include=None, cmap='tab10', f_threshold=150):
    ...
    det_type, network, drive, sense, bias = data_cols#, coldload_temp, bath_temp = data_cols
    tones = include if include is not None else det.stream.tones
    if isinstance(tones, int): tones = [tones]
    fig_dir = Path(sess_dir) / 'fig' / f'B{bid}D{drid}' / 'LTD' 
    if not fig_dir.exists(): rfsoc_io.create_dir(fig_dir)
    fig_file = fig_dir / f'det_dashboard_D{drive}_S{sense}_B_{bias:0.1f}_{det.stream.timestamp}'
    
    spawn_context = mp.get_context('spawn')
    det._properties_df = det._properties_df.drop(pl.col(pl.Object))
    #min(48, int(len(tones)/2)
    dark_blues = mpl.colors.LinearSegmentedColormap.from_list("dark_blues", ['#A6CEE3', '#0072B2'], N=256)

    det.vna.mag(dB=True)
    vna_mag, vna_mag_df = det.vna.mag_plot(prefix='dB', return_df=True)
    vna_mag = vna_mag_df.with_columns(pl.col('f')/1e6).hvplot.line(x='f', y='mag', fontscale=3, pixel_ratio=0.5, width=4*viz_cfg['plot']['width'], height=viz_cfg['plot']['height'], title='VNA Sweep', xlabel=r'$Frequency\ [MHz]$', ylabel=r'$|S_{21}|\ [dB]$', datashade=True, cmap=dark_blues, linewidth=4)
    


    for tone in tones:
        targ_f = det.targ.get_data('f', include=tone)
        min_f, max_f = targ_f.select(pl.col(f'f_{tone:04d}').first().alias('min'), pl.col(f'f_{tone:04d}').last().alias('max'))[0].to_numpy().T.flatten()

        vna_tone_fig = vna_mag*hv.VSpan(min_f/1e6, max_f/1e6).opts(color='#D55E00')
        vna_fig = hv.render(vna_tone_fig, backend='matplotlib')
        vna_fig.savefig(fig_dir / f'vna_{tone:04d}.svg', bbox_inches="tight", pad_inches=0.1)
        plt.close(vna_fig)
        _plot_det_dashboard(tone, det, cmap, viz_cfg, fig_file, det_type, network, drive, sense, bias)

    
    # with concurrent.futures.ProcessPoolExecutor(max_workers=1, mp_context=spawn_context) as executor:
    #     futures = [executor.submit(_plot_det_dashboard, tone, det, cmap, viz_cfg, fig_file, det_type, network, drive, sense, bias) for tone in tones]#, coldload_temp, bath_temp) for tone in tones]
...
def plot_property_hists(property_df, viz_cfg, data_cols, labels, bins = 100):
    ...
    for i, (data_col, label) in enumerate(zip(data_cols, labels)):
        try:
            data_std, data_med = (property_df.select(pl.col(data_col).std().alias('std'),
                                                                pl.col(data_col).median().alias('mean'))[0].to_numpy().T.flatten())
            df = property_df.select(data_col).filter((pl.col(data_col) > data_med - 2*data_std) & (pl.col(data_col) < data_med + 2*data_std))
...
def _plot_det_dashboard(tone, det, cmap, viz_cfg, fig_file, det_type, network, drive, sense, bias):#, coldload_temp, bath_temp):
    try:
        # Plot |S21|
        # ==========
        ...
        targ_mag_fit_fig = targ_mag_fit.NdOverlay.Curve[tone].opts(opts.Curve(color='#D55E00', linewidth=1.5)).relabel(label='Resonator Fit', group='Sweep')
        targ_mag_fig = targ_mag.NdOverlay.Scatter[tone].opts(opts.Scatter(color='#0072B2')).relabel(label='Sweep Data', group='Sweep')
        # Plot |S21| timestream
        ...
        stream_mag = stream_mag[tone].relabel(label='Stream Data', group='Stream')
        ...
        mag_fig = targ_mag_fit_fig*targ_mag_fig#*stream_mag.opts(alpha=0.7, s =5#)
        mag_fig.opts(ylabel=r'$|S_{21}|\ [dB]$', title='Target Sweep Magnitude', legend_position='top_right')
        ...
        targ_phase_fit, targ_phase_fit_df = det.targ.phase_plot(prefix='mismatch_rotate_origin_shift_origin_rotate_unwind_rotate_nonlinear_fit', include=tone, return_df = True)
        targ_phase_to_f_spline, targ_phase_to_f_spline_df = det.targ.plot('spline', 'phase', x_prefix='mismatch_rotate_origin_shift_origin_rotate_unwind_rotate_phase_to_f', y_prefix='mismatch_rotate_origin_shift_origin_rotate_unwind_rotate', include=tone)
        targ_f_to_phase_spline, targ_f_to_phase_spline_df = det.targ.plot('f', 'spline', x_prefix='', y_prefix='f_to_mismatch_rotate_origin_shift_origin_rotate_unwind_rotate_phase', include=tone)
        
        if not (targ_phase_to_f_spline_df['spline'] == 0).all():
            targ_phase_spline_fig = targ_phase_to_f_spline.NdOverlay.Curve[tone].opts(opts.Curve(color='#009E73', linewidth=2)).relabel(label='Spline', group='Sweep')
        elif not (targ_f_to_phase_spline_df['spline'] == 0).all():
            targ_phase_spline_fig = targ_f_to_phase_spline.NdOverlay.Curve[tone].opts(opts.Curve(color='#009E73', linewidth=2)).relabel(label='Spline', group='Sweep')
        else:
            targ_phase_spline_fig = hv.Scatter([])

        targ_phase_fit_fig = targ_phase_fit.NdOverlay.Curve[tone].opts(opts.Curve(color='#D55E00', linewidth=1.5)).relabel(label='Resonator Fit', group='Sweep') 
        targ_phase_fig = targ_phase.NdOverlay.Scatter[tone].opts(opts.Scatter(color='#0072B2')).relabel(label='Sweep Data', group='Sweep')
        ...
        stream_phase = stream_phase[tone].relabel(label='Stream Data', group='Stream')
        ...
        phase_fig = targ_phase_fit_fig*targ_phase_fig#*stream_phase.opts(alpha=0.5, s=5)
        ...
        circle_fit_fig = circle_fit_IQ.NdOverlay.Curve[tone].opts(opts.Curve(color='#D55E00', linewidth=1.5)).relabel(label='Circle Fit', group='Sweep')
        targ_IQ_fig = targ_IQ.NdOverlay.Scatter[tone].opts(opts.Scatter(color='#0072B2')).relabel(label='Sweep Data', group='Sweep')
        # Plot IQ timestream
        ...
        stream_IQ = stream_IQ[tone].relabel(label='Stream Data', group='Stream')
        ...
        IQ_fig = circle_fit_fig*hv.HLine(0).opts(color='k', linewidth=0.5)*hv.VLine(0).opts(color='k', linewidth=0.5)*targ_IQ_fig#*stream_IQ.opts(alpha=0.7)
        ...
        row_1 = mag_fig + IQ_fig + phase_fig
        ...
        timestream_I, timestream_I_df = det.stream.stream_plot('I', prefix='timestream_rotate_origin_shift_origin_rotate_unwind_rotate', include=tone, return_df = True, rasterize=False)
        timestream_I_df = timestream_I_df.with_columns([(pl.col('t') - pl.col('t').first()).alias('t'), (pl.col('I') - pl.col('I').mean()).alias('I')])

        timestream_Q, timestream_Q_df = det.stream.stream_plot('Q', prefix='timestream_rotate_origin_shift_origin_rotate_unwind_rotate', include=tone, return_df = True, rasterize=False)
        timestream_Q_df = timestream_Q_df.with_columns([(pl.col('t') - pl.col('t').first()).alias('t'), (pl.col('Q') - pl.col('Q').mean()).alias('Q')])

        
        dark_blues = mpl.colors.LinearSegmentedColormap.from_list("dark_blues", ['#A6CEE3', '#0072B2'], N=256)
        verm_reds = mpl.colors.LinearSegmentedColormap.from_list("verm_reds", ['#F4B19D', '#D55E00'], N=256)
        ...
        timestream_I_Q = timestream_IQ_df.hvplot.line(x='t', y='Q', linewidth=3, dynamic=False, cmap=dark_blues, width=viz_cfg['plot']['width'], height=viz_cfg['plot']['height'], datashade=True)*timestream_IQ_df.hvplot.line(x='t', y='I', dynamic=False, cmap=verm_reds, width=viz_cfg['plot']['width'], linewidth=3, height=viz_cfg['plot']['height'], datashade=True)
        I_Q_legend = hv.Curve((0,0), label='Dissipation Quadrature (I)').opts(color='#D55E00')*hv.Curve((0,0), label='Frequency Quadrature (Q)').opts(color='#0072B2')
        timestream_I_Q = (I_Q_legend*timestream_I_Q).opts(title='I & Q Timestreams')
        ...
        stream_centered_IQ = stream_centered_IQ.NdOverlay.Scatter[tone]
        stream_centered_IQ = datashade(stream_centered_IQ, dynamic=False, cnorm='eq_hist', cmap=dark_blues, pixel_ratio=0.1, width=viz_cfg['plot']['height'], height=viz_cfg['plot']['height']).opts(aspect=1)
        ...
        stream_centered_IQ.opts(xlim=(-stream_abs_max, stream_abs_max), ylim=(-stream_abs_max, stream_abs_max), xlabel=r'$I\ [arb]$', ylabel=r'$Q\ [arb]$', title='Centered IQ Timestream', aspect=1, show_legend=False)
        ...
        psd_I_fig = psd_I.NdOverlay.Curve[tone].opts(linewidth=2, color='#D55E00').relabel('Dissipation Quadrature (I)')
        psd_Q_fig = psd_Q.NdOverlay.Curve[tone].opts(linewidth=2, color='#0072B2').relabel('Frequency Quadrature (Q)')
        psd_IQ_fig_legend = psd_I_df[0].hvplot.scatter('psd_f', 'I', label=f"Q/I Ratio: {det_properties.select('freq_diss_ratio').item():0.2f}").opts(alpha=0)
        ...
        psd_IQ_fig.opts(ylabel=r'$PSD\ \left[\sqrt{arb/Hz}\right]$', title='I & Q PSDs', logx=True, logy=True)
        psd_x_fig.opts(ylabel=r'$\sqrt{S_{xx}}\ \left[Hz^{-1/2}\right]$', title='Fractional Frequency PSD', logx=True, logy=True)
        row_3 = (stream_centered_IQ*hv.HLine(0).opts(color='k', linewidth=0.5)*hv.VLine(0).opts(color='k', linewidth=0.5) + timestream_I_Q.opts(xlabel=r'$Time\ [s]$', ylabel=r'$I,\ Q\ [arb]$') + psd_IQ_fig).opts(shared_axes=False)

        fig = (row_1  + row_3).cols(3).opts(shared_axes=False)
        mpl_fig = hv.render(fig, backend='matplotlib')
        #mpl_fig.suptitle(f'{det_type} {network} Tone {tone}: Drive {drive} dB, Sense {sense} dB, LNA Bias {bias} mA')#, Coldload Temp {coldload_temp} K, Bath Temp {bath_temp} mK')
        print('saving')
        save_file = f'{fig_file}_{tone:04d}.svg'
        mpl_fig.savefig(save_file, bbox_inches="tight", pad_inches=0.1)
        plt.close(mpl_fig)
...
def center_IQ(detector, data = 'both', delay_col='cable_delay', recalc=False):
    ...
    detector.IQ_circle_fit(prefix='unwind_rotate', max_workers=1, recalc=recalc)
...
def phase_to_f(detector, recalc = False, phase_bounds=0.2, k=2, nperseg=512, f_threshold=150):
...
def center_timestream(detector, recalc=False, nperseg=512, f_threshold=150):
