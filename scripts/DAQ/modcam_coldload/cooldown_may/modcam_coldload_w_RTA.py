...
def main():
    ...
    RC = R(cfg_path = "./system_config.yaml")
    ...
    # Run coldload sweep
    coldload_sweep(RC, coldload_cfg, attens, noise_attens, restart = restart)
...
def coldload_sweep(R, coldload_cfg, attens, noise_attens, restart = False):
    ...
    psu = OCSClient('power-psu-BKP9130B', args = [])
    psu.init.start()
    psu.init.wait()
    ...
    while remaining>0:
        ...
        with tqdm(range(len(currents)), desc = f'DAQ:') as pbar:
            for i in pbar:
                ...
                R._edit_config(R.ext_cfg, "power_supply_current", curr)
                R._edit_config(R.ext_cfg, "power_supply_voltage", volt)
...
def update_logs(R, coldload_agent, fp_agent, logname):
    ...
    R._edit_config(R.ext_cfg, "coldload_temp", str(coldload_temp))
    R._edit_config(R.ext_cfg, "array_temp", array_temps)
    R._edit_config(R.ext_cfg, "temp_time", timestamp)
