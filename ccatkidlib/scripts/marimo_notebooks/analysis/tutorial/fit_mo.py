...
def _(PREFIX, det, mismatch_prefix):
    phase_plot_df = det.targ.phase_plot(
        prefix=f"{PREFIX['phase_fit']}_{mismatch_prefix}",
...
@app.cell(hide_code=True)
def _(mo, phase_fit_plot, tone_selector):
    mo.md(rf"""
    ### Detector Phase Fit

    {mo.lazy(mo.vstack([tone_selector, phase_fit_plot]))}
    """)
    return
...
def _():
    ...
    return Detector, ccat_io, routines
