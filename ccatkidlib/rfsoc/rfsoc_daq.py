...
from ccatkidlib.log import header, Style
...
class R:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        new_measurement = (
            sess_id is None
            or curr_date != self.curr_date
            or self.measurement_name != self.io_cfg["name"]
            or self.measurement_desc != self.io_cfg["desc"]
        )
        self.sess_id = str(time.time())[:10] if new_measurement else sess_id

        # Save session ID to config files
        # -------------------------------
        io.edit_config(self.ext_cfg, "sess_id", self.sess_id)
        io.edit_config(self.io_cfg, "sess_id", self.sess_id)
        for cfg in self.drone_cfg:
            io.edit_config(cfg, "sess_id", self.sess_id)

        # Create directories
        # ------------------
        new_dir_paths = io.create_tree(
            self.drone_list, self.curr_date, self.sess_id, self.data_dir
        )  # Create file directory structure for saving data
        self.config_dirs, self.targ_dirs, self.timestream_dirs, self.vna_dirs = (
            new_dir_paths
        )
        self.noise_files = io.create_noise_files(
            self.drone_list, self.tmp_dir
        )  # Create tmp directory and noise tone files

        # Setup logger
        self.log_dir = self.config_dirs[0].parent
        log.setup_logging(
            self.log_dir / self.io_cfg["io"]["logging_fname"],
            self.io_cfg["io"]["file_level"],
            self.io_cfg["io"]["terminal_level"],
            max_file_size=self.io_cfg["io"]["max_size"],
        )
        log.log(
            "INFO",
            f"{Style.INVERT}Date: {self.curr_date}; Session: {self.sess_id}{Style.DEFAULT}",
        )
        ...
        log.log(
            "DEBUG", f"Finished appending file paths! Paths in sys.path: {sys.path}"
        )
        ...
        init_boards = (
            init_boards
            if init_boards is not None
            else self.io_cfg["init"]["initialize_boards"]
        )
        if init_boards:
            self.setup_boards()
        ...
        init_drones = (
            init_drones
            if init_drones is not None
            else self.io_cfg["init"]["initialize_drones"]
        )
        if init_drones:
            self.setup_drones()
        log.log("INFO", f"Communicating with drones: {self.drone_list}!")
        ...
        # -----------------
        io.save_config(
            self.log_dir / f"init_config_ext_{self.timestamp}.yaml",
            self.ext_cfg,
            self.save_cfg,
        )
        io.save_config(
            self.log_dir / f"init_config_io_{self.timestamp}.yaml",
            self.io_cfg,
            self.save_cfg,
        )
        for rfsoc_dir, cfg in zip(self.config_dirs, self.drone_cfg):
            io.save_config(
                rfsoc_dir / f"init_config_drone_{self.timestamp}.yaml",
                cfg,
                self.save_cfg,
            )
    ...
    def _update_measurement(
        self, name: str | None = None, desc: str = "", influx_output: bool | None = None
    ) -> tuple[str, str] | None:
        """
        ...
        influx_output = (
            influx_output
            if influx_output is not None
            else self.io_cfg["io"]["influx_output"]
        )

        if influx_output:
            sess = self.rfsoc.updateMeasurement(
                measurement_name=f"{name}, Date: {self.curr_date}, Session ID: {self.sess_id}"
                if name
                else None,
    ...
    def _load_system_config(self, cfg_path: str) -> None:
        ...
        try:
            ...
            print(
                "System config must contain two config files (an external config and an IO config)! Please reference the example system config file."
            )
            sys.exit()
        ...
        self.drone_list, bids = autils.get_com_to(
            self, com_to=self.drone_list, setup=False
        )
        ...
        for board in board_cfgs:
            ...
            if board[1:] in self.board_list:
                ...
                self.drone_cfg.append(io.load_config(board_cfgs[board]["drone_cfg"]))
    ...
    def setup_boards(self, **kwargs) -> None:
        ...
        for board in boards:
            ...
            with io.get_connection(bip, ssh_key) as c:
                ...
                # ------------------
                self.setup_drones(
                    com_to=[f"{board}.1", f"{board}.2", f"{board}.3", f"{board}.4"],
                    restart=True,
                )
    ...
    def setup_drones(self, **kwargs) -> None:
        ...
        master_drone = io.load_config(master_drone_file)
        ...
        # Save edited config
        io.save_config(master_drone_file, master_drone)
        ...
        for board in self.all_boards:
            ...
            for i in range(4):
                ...
                ip, to_run, running = [
                    substr.split("=")[-1]
                    for substr in json.loads(rtn.session["data"]["data"]).split(", ")
                ]
        ...
        if wait:
            ...
            for board in self.all_boards:
                ...
                for i in range(4):
                    com = f"{board}.{i + 1}"  # bid.drid
                    rtn = self.rfsoc.action(
                        com_to=com, action="status"
                    )  # Get drone status
                    ...
                    to_run, running = (
                        ast.literal_eval(to_run),
                        ast.literal_eval(running),
                    )
                    log.log("PCS", f"{rtn.session}")
                    ...
                    if to_run:
                        if running:
                            ...
                            log.log("ERROR", f"Failed to start drone {com}")

        # Log the currently running drones at INFO level if a change was made otherwise log to DEBUG
        log.log(
            "INFO" if wait else "DEBUG",
            f"Drones {sorted(running_drones)} are currently running!",
        )
    ...
    def set_NCLO(self, NCLO: int | list[int] | None = None, **kwargs) -> list[float]:
        ...
        def _set_NCLO(com: str, *args, **kwargs):
            ...
            """
            NCLO = args[0]
            rtn = self.rfsoc.setNCLO(
                com_to=com, f_lo=NCLO, silent=True
            )  # Set NCLO frequency
            log.log("PCS", f"{rtn.session}")
            return rtn
        ...
        # --------------------------------
        NCLOs = autils.get_drone_args(
            self, com_to, ["tones", "NCLO"]
        )  # Get NCLOs from drone config files

        # Override config attenuations with those passed as method argument (if any)
        if NCLO is not None:
            LOs = autils.parse_args(
                self, com_to, NCLO
            )  # Parse NCLOs passed as argument
            if LOs is None:
                return None  # Return None if invalid NCLOs were passed

            NCLOs = list(
                map(int, LOs)
            )  # Parse NCLOs passed as argument and cast as int

            autils.set_drone_args(
                self, com_to, "NCLO", NCLOs
            )  # Update NCLOs in drone config files
        ...
        log.log(
            "INFO", f"Successfully set NCLO to {list(NCLOs)} for drones {list(com_to)}!"
        )
    ...
    def set_atten(
        self,
        drive: float | list[float] | None = None,
        sense: float | list[float] | None = None,
        **kwargs,
        ...
        def _set_direction(
            com_to: list[str], direction: str, atten: list[float] = None
        ) -> list[float] | None:
            """
            ...
            def _set_atten(com: str, *args, **kwargs):
                ...
                rtn = self.rfsoc.setAtten2025(
                    com_to=com, direction=direction, atten=att, silent=True
                )
                log.log("PCS", f"{rtn.session}")
                return rtn
            ...
            # -----------------------
            attens = autils.get_drone_args(
                self, com_to, ["atten", f"{direction}"]
            )  # Get attenuations from drone config files

            # Override config attenuations with those passed as method argument (if any)
            if atten is not None:
                attens = list(
                    map(float, autils.parse_args(self, com_to, atten))
                )  # Parse attenuations passed as argument and cast to int
                ...
                autils.set_drone_args(
                    self, com_to, direction, attens
                )  # Update attenuations in drone config files
            ...
            new_com_to = []
            new_dict = getattr(
                self, f"{direction}_attens"
            )  # Get current attenuations for the specified direction
            for com, atten in zip(com_to, attens):
                if not new_dict.get(com, None) == atten:
                    ...
                    new_dict[com] = atten
            setattr(
                self, f"{direction}_attens", new_dict
            )  # Update current attenuations for the specified direction
            ...
            # log.log('INFO', f'Successfully set {direction} attenuation to {list(attens)} for drones {list(com_to)}!')
        ...
        def _check_direction(
            com_to: list[str],
            direction: str,
            set_attens: list[float],
            new_attens: list[float],
        ) -> None:
            """
            ...
            for com, set_atten, new_atten in zip(com_to, set_attens, new_attens):
                if new_atten != set_atten:
                    log.log(
                        "ERROR",
                        f"Failed to set {direction} attenuation for drone {com} to {set_atten} dB. \
        ...
        _check_direction(
            com_to, direction="drive", set_attens=set_drives, new_attens=new_drives
        )
        log.log(
            "INFO",
            f"Successfully set drive attenuations to {new_drives} for drones {com_to}.",
        )

        _check_direction(
            com_to, direction="sense", set_attens=set_senses, new_attens=new_senses
        )
        log.log(
            "INFO",
            f"Successfully set sense attenuations to {new_senses} for drones {com_to}.",
        )
    ...
    @log.method_timer
    def get_stats(
        self, space: bool = True, temps: bool = True, ADC_rms: bool = True, **kwargs
    ) -> tuple[list[float], tuple[list[float], list[float]], list[float]]:
        """
        ...
        if kwargs["setup"]:
            self.setup_drones(**kwargs)  # Setup drones
        ...
        if not self.rfsoc.feedMonitor.status().session["op_code"] == 2:
            ...
            log.wait(60, desc="HK Feed Monitor Starting")

        avail_spaces, temp_lists, rms_list = [], [], []
        kwargs["setup"] = False  # Do not set up drones again
        if space:
            avail_spaces = self.get_avail_space(**kwargs)  # Get storage space
        if temps:
            temp_lists = self.get_temps(**kwargs)  # Get temperatures
        if ADC_rms:
            rms_list = self.get_ADC_rms(**kwargs)  # Get RMS power at ADCs
    ...
    def get_ADC_rms(self, **kwargs) -> list[float]:
        ...
        for rtn in rtns:
            ...
            ind = self.drone_list.index(
                f"{data_dic['bid']}.{data_dic['drid']}"
            )  # Determine which drone the Snap data corresponds to
            inds.append(ind)
            ...
            )  # Calculate RMS value at the ADC
            rms_list.append(rms)

            io.edit_config(
                self.drone_cfg[ind], "ADC_RMS", rms
            )  # Add ADC rms to drone config

        # Create list of ADC rms values for all drones, sort by drone bid.drid
        rms_list = [rms for _, rms in sorted(zip(inds, rms_list))]
        log.log("INFO", f"RMS power at the ADC is {rms_list} for drones {com_to}!")
        return rms_list
    ...
    def get_avail_space(self, **kwargs) -> list[float]:
        ...
        def _get_space(bids) -> list[float]:
            ...
            for bid in bids:
                ...
                io.edit_config(
                    self.ext_cfg,
                    ["boards", f"b{bid}", "avail_space"],
                    avail_space,
                    append=True,
                )
            log.log("INFO", f"Storage space is {avail_spaces} GB for boards {bids}!")
            return avail_spaces

        com_to, bids = autils.get_com_to(self, **kwargs)
        kwargs["setup"] = False

        threshold = self.io_cfg["clean"][
            "threshold"
        ]  # [GB] Threshold after which clean board drone directories
        ...
        if threshold > 0:
            boards_to_clean = [
                bid
                for avail_space, bid in zip(avail_spaces, bids)
                if avail_space < threshold
            ]
            if len(boards_to_clean) > 0:
                drones_to_clean = [
                    com for com in com_to if com.split(".")[0] in boards_to_clean
                ]
                log.log("INFO", f"Cleaning drones {drones_to_clean}...")
                for com in drones_to_clean:
                    self.rfsoc.cleanBoardDroneDirs(
                        com_to=com,
                        testing=False,
                        leave_latest=True,
                        olderThanDaysAgo=str(olderThanDaysAgo),
                    )
                log.log("INFO", f"Finished cleaning drones {drones_to_clean}")
                avail_spaces = _get_space(bids)

                if self.io_cfg["clean"]["tmp"]:
                    # Clean primecam_readout tmp directory
                    clean_io.cleanQueenTmpDir(
                        testing=False, ftype="", olderThanDaysAgo=str(olderThanDaysAgo)
                    )

                    # Clean ccatkidlib tmp directory
                    ignore_list = [
                        str(Path(self.tmp_dir).resolve() / "noise_tones.npy")
                    ]
                    clean_io._cleanDir(
    ...
    def get_temps(self, **kwargs) -> tuple[list[float], list[float]]:
        ...
        def _get_temps(bids: list[str], loc: str) -> list[float]:
            ...
            for bid in bids:
                ...
                temp = float(
                    np.mean(
                        [temp_dict[f"temp_{bid}_{drid + 1}_{loc}"] for drid in range(4)]
                    )
                )
                temps.append(temp)
                io.edit_config(
                    self.ext_cfg,
                    ["boards", f"b{bid}", f"{loc}_temp"],
                    temp,
                    append=True,
                )
            log.log("INFO", f"{loc} temperatures are {temps} \xb0C for boards {bids}!")
            return temps
    ...
    def get_atten(self, **kwargs) -> tuple[list[float], list[float]]:
        ...
        def _get_direction(com_to: str | list[str], direction: str) -> list[float]:
            ...
            def _get_atten(com: str, *args, **kwargs):
                ...
                direction = args[0]
                rtn = self.rfsoc.getAtten(
                    com_to=com, direction=direction, silent=False
                ).session
                log.log("PCS", f"{rtn}")
                return json.loads(rtn["data"]["data"])[1:][0]
            ...
            for rtn in rtns:
                ...
                if not isinstance(atten, float):
                    ...
                    log.log(
                        "ERROR",
                        f"Failed to get {direction} attenuation for drone {com}.",
                    )
                inds.append(ind)
                attens.append(atten)
            ...
            log.log(
                "INFO",
                f"Successfully got {direction} attenuations {attens} for drones {com_to}!",
            )
    ...
    def take_vna_sweep(self, **kwargs) -> list[str]:
        ...
        # --------------
        if write_comb:
            log.log("INFO", "Writing new VNA combs...")
            self._run_parallel(_write_vna_comb, **kwargs)
            log.log("INFO", f"Successfully wrote new VNA combs for drones {com_to}!")
            time.sleep(
                1
            )  # Wait before taking sweep (not waiting can affect sweep quality, unsure if this is still the case: need to test)

        # Take VNA sweep
        # --------------
        self.timestamp = str(time.time()).split(".")[0]
        log.log("INFO", "Taking VNA sweeps...")
        ...
        log.log(
            "INFO",
            f"Finished taking VNA sweeps for drones {com_to} with timestamp {self.timestamp}!",
        )
    ...
    def take_target_sweep(self, **kwargs) -> list[str]:
        ...
        def _take_targ_sweep(com: str, *args, **kwargs):
            ...
            rtn = self.rfsoc.targetSweep(
                com_to=com, sweep_steps=sweep_steps, chan_bw=chan_bw, silent=False
            )
            log.log("PCS", f"{rtn.session}")
            return rtn
        ...
        # Evaluate kwargs
        write_comb = any(
            key in kwargs for key in ["tone_powers", "tone_freqs", "tone_phis"]
        )
        sweep_steps = autils.get_drone_args(self, com_to, ["tones", "sweep_steps"])
        ...
        if write_comb:
            ...
            time.sleep(
                1
            )  # Wait before taking sweep (not waiting can affect sweep quality)

        # Get timestamp
        self.timestamp = str(time.time()).split(".")[0]
        log.log("INFO", "Taking target sweeps!")
        ...
        log.log(
            "INFO",
            f"Finished taking target sweeps for drones {com_to} with timestamp {self.timestamp}!",
        )
        self.get_stats(**kwargs)
    ...
    def take_timestream(self, t_sec: float, **kwargs) -> list[str]:
        ...
        def _take_timestream_numpy(com: list[str], *args, **kwargs) -> list[str]:
            ...
            def _save_timestream_numpy(
                com_to, start_time, time_diff, packet_data, tstream_seg, tstream_outs
            ):
                ...
                data, auxs, time_diff.value = self.streamer.parse_packets(
                    packs, timestamp=start_time, time_diff=diff
                )
                del packs, addrs
                gc.collect()
                ...
                for dat, aux, ip in zip(data, auxs, ips):
                    ...
                    drone_data[drone_ind].append(
                        np.append(
                            np.int64(aux[2]),
                            np.append(aux[3], np.array(dat).astype("int64")),
                        )
                    )
                ...
                for com, data in zip(com_to, drone_data):
                    if not len(data) == 0:
                        ...
                        if self.save_data:
                            ...
                            tstream_file = (
                                timestream_dir / f"{fname}_{tstream_seg:03d}.npy"
                            )
            ...
            q = Queue(maxsize=0)
            packet_capture = Process(
                target=self.streamer.capture_packets, args=(t_sec,), kwargs={"q": q}
            )
            timer = Process(
                target=log.wait,
                args=(t_sec,),
                kwargs={"desc": f"Taking {t_sec} second timestream for {com}"},
            )

            with Manager() as manager:
                tstream_outs = manager.list(
                    [manager.list([]) for _ in range(len(com_arr))]
                )
                time_diff = Value("d", -1)
                tstream_seg = 0
                ...
                log.log(
                    "INFO",
                    f"Finished taking {t_sec} seconds of timestream data with timestamp {self.timestamp}!",
                )
        ...
        def _take_timestream_g3(com: list[str], *args, **kwargs):
            ...
            t_sec = args[0]
            rtn = self.rfsoc.timestreamOn(com_to=com, on=True, silent=True)
            log.wait(t_sec, desc=f"Taking {t_sec} second timestream for {com}")
            rtn = self.rfsoc.timestreamOn(com_to=com, on=False, silent=True)
            log.log(
                "INFO",
                f"Finished taking {t_sec} seconds of timestream data with timestamp {self.timestamp}!",
        ...
        def _save_timestream_g3(com_to: list[str]) -> list[str]:
            ...
            for com in com_to:
                ...
                g3_file = io.get_most_recent_file(
                    g3_tstream_dir, f"r{int(bid):02d}d{drid}*", time_past=np.inf
                )
                ...
                while (
                    tstream_num >= 0
                    and io.get_creation_time(g3_file) - float(self.timestamp) > 0
                ):
        ...
        # Take timestreams
        # ----------------
        log.log("INFO", f"Taking {t_sec} seconds of timestream data!")
        stream_out = None
        self.timestamp = str(time.time()).split(".")[0]
        args = [t_sec]
        if g3:
            ...
            if self.streamer is None:
                self.streamer = Streamer(
                    self.io_cfg["timestream"]["udp_ip"],
                    self.io_cfg["timestream"]["udp_port"],
                )
                log.log(
                    "INFO",
                    f"Successfully initialized timestream object using address {self.io_cfg['timestream']['udp_ip']} and port {self.io_cfg['timestream']['udp_port']}!",
                )
        ...
        # --------------------------------------------------------
        for com in com_to:
            self._save_curr_comb(com, self.io_cfg["save_file_names"]["timestream"])

        # Save ext config
        # ---------------
        self.ext_cfg = io.save_config(
            self.log_dir
            / f"{self.io_cfg['save_file_names']['timestream']}_config_ext_{self.timestamp}.yaml",
            self.ext_cfg,
            self.save_cfg,
        )
    ...
    def write_config_comb(self, **kwargs):
        ...
        tone_freqs = autils.get_drone_args(
            self, com_to, ["tones", "custom", "tone_freqs"]
        )
        tone_powers = autils.get_drone_args(
            self, com_to, ["tones", "custom", "tone_powers"]
        )
        tone_phis = autils.get_drone_args(
            self, com_to, ["tones", "custom", "tone_phis"]
        )
        rescale_power = autils.get_drone_args(
            self, com_to, ["tones", "custom", "generation", "rescale_power"]
        )
        gen_attempts = autils.get_drone_args(
            self, com_to, ["tones", "custom", "generation", "gen_attempts"]
        )
        ...
        for key, value in kwargs.items():
            if key == "tone_freqs":
                ...
                autils.set_drone_args(
                    self,
                    com_to,
                    ["tones", "custom", "generation", "rescale_power"],
                    rescale_power,
                )
                ...
                autils.set_drone_args(
                    self,
                    com_to,
                    ["tones", "custom", "generation", "gen_attempts"],
                    gen_attempts,
                )

        # Write custom comb for each drone
        # --------------------------------
        for com, rescale, attempts, freq, power, phi in zip(
            com_to, rescale_power, gen_attempts, tone_freqs, tone_powers, tone_phis
        ):
            self._write_custom_comb(
                com,
                rescale_power=rescale,
        ...
        log.log("INFO", f"Sucessfully wrote custom comb for drones {com_to}!")
        return rtn
    ...
    def find_detectors(self, new_sweep: bool = True, **kwargs):
        ...
        def _write_filtered_comb(com_to, vna_files, save_files, filter_wn):
            ...
            for i, (com, vna_file, save_file, win) in enumerate(
                zip(com_to, vna_files, save_files, filter_wn)
            ):
                # Filter out fake resonators from found detectors
                # -----------------------------------------------
                vna = VNA(com_to=com, data_path=vna_file)  # Create VNA object

                good_resonator, _ = vna.filter_det_f(
                    win=win
                )  # Filter out fake resonators
                ...
                io.edit_config(drone_cfg, "found_num_detectors", found_num)
                io.edit_config(
                    drone_cfg, "found_detector_freqs_filtered", str(save_path)
                )
                log.log(
                    "INFO",
                    f"Kept {found_num} detectors after filtering for drone {com}!",
                )

                drone_cfg = io.save_config(save_file, drone_cfg, self.save_cfg)

            # Write a new target comb using the filtered resonator frequencies
            # ----------------------------------------------------------------
            rtn = self.write_config_comb(
                com_to=com_to,
                tone_freqs=good_resonators,
                tone_powers=num_drones * ["gen"],
                tone_phis=num_drones * ["gen"],
                rescale_power=True,
                gen_attempts=1,
            )
        ...
        # -------------------------------------
        peak_prom_std = autils.get_drone_args(
            self, com_to, ["det_find", "peak_prom_std"]
        )
        ...
        for i, com in enumerate(com_to):
            ...
            vna_file = io.get_most_recent_file(
                self.vna_dirs[ind],
                f"{self.io_cfg['save_file_names']['vna_sweep'][0]}*",
                time_past=24 * 3600,
            )
        ...
        if len(to_sweep) != 0:
            ...
            for i in range(len(com_to)):
                if vna_files[i] is None:
                    vna_files[i] = new_files.pop(
                        0
                    )  # Add old vna files into list of new vna files sorted by drone com_to
                if timestamps[i] == -1:
                    timestamps[i] = self.timestamp
            self.save_cfg = True
            kwargs["com_to"] = com_to

        # Find resonators using VNA sweep(s)
        # ----------------------------------
        log.log("INFO", f"Finding resonators from VNA sweep for drones {com_to}!")
        for i, com in enumerate(com_to):
            # Find resonators from VNA sweep
            rtn = self.rfsoc.findVnaResonators(
                com_to=com,
                peak_prom_std=peak_prom_std[i],
                peak_prom_db=peak_prom_db[i],
                peak_dis=peak_dis[i],
                width_min=width_min[i],
                width_max=width_max[i],
                stitch=stitch[i],
                stitch_bw=stitch_bw[i],
                stitch_sw=stitch_sw[i],
                remove_cont=remove_cont[i],
                continuum_wn=continuum_wn[i],
                remove_noise=remove_noise[i],
                noise_wn=noise_wn[i],
                silent=False,
            )
            log.log("PCS", f"{rtn.session}")
        log.log(
            "INFO", f"Finished finding resonators from VNA sweep for drones {com_to}!"
        )

        # Save found resonators files
        # ---------------------------
        found_nums, found_freqs, save_files = self._save_resonators(
            com_to, timestamps, "vna"
        )

        if write_targ_comb:
            log.log(
                "INFO",
                f"Writing target combs using found resonators for drones {com_to}!",
            )
            filt_com_to, unfilt_com_to = (
                np.array(com_to)[phase_filter],
                np.array(com_to)[np.logical_not(phase_filter)],
            )

            if (
                not len(filt_com_to) == 0
            ):  # Write target comb with phase filtered resonators
                log.log("INFO", f"Filtering resonators for drones {filt_com_to}!")
                rtn, found_nums = _write_filtered_comb(
                    filt_com_to,
                    np.array(vna_files)[phase_filter],
                    np.array(save_files)[phase_filter],
                    np.array(filter_wn)[phase_filter],
                )  # TODO: Might want to seperate filtering and comb writing

            if (
                not len(unfilt_com_to) == 0
            ):  # Write target comb using all found resonators
                kwargs["com_to"] = unfilt_com_to
                rtn = self._run_parallel(_write_targ_comb, **kwargs)
            log.log("INFO", f"Finished writing target combs for drones {com_to}!")

            # Saved target comb has no noise tones so overwrite noise tone files
            for com in com_to:
                np.save(self.noise_files[self.drone_list.index(com)], [])

            # Write current comb to drone config files without saving to disk
            self.save_cfg = False
            for com in com_to:
                self._save_curr_comb(com, None)
            self.save_cfg = True
    ...
    def tune_tone_placement(self, new_sweep: bool = True, **kwargs):
        ...
        def _place_min(com_to, com_inds):
            ...
            log.log(
                "INFO", f"Finding resonators from target sweep for drones {com_to}!"
            )
            for i, com in zip(com_inds, com_to):
                # Find resonators from target sweep
                rtn = self.rfsoc.findTargResonators(
                    com_to=com, stitch_bw=stitch_bw[i], silent=False
                )
                log.log("PCS", f"{rtn.session}")
            log.log(
                "INFO",
                f"Finished finding resonators from target sweep for drones {com_to}!",
            )

            found_nums, found_freqs, save_files = self._save_resonators(
                com_to, np.array(timestamps, dtype=int)[com_inds], "targ"
            )
            return [[]] * len(com_to)
        ...
        def _place_grad(com_to, com_inds):
            ...
            for i, (com_ind, com, timestamp) in enumerate(
                zip(com_inds, com_to, com_timestamps)
            ):
                wn, trim_savgol_wn, diff_savgol_wn = (
                    window[com_ind],
                    trim_savgol_window[com_ind],
                    diff_savgol_window[com_ind],
                )
                trim_savgol_k, diff_savgol_k = (
                    trim_savgol_order[com_ind],
                    diff_savgol_order[com_ind],
                )
                ...
                targ = Target(
                    com_to=com, data_path=targ_files[com_ind], tones=range(num_tones)
                )
                ...
                found_nums[i] = found_num
                io.edit_config(drone_cfg, "found_num_detectors", found_num)
                io.edit_config(drone_cfg, "found_detector_freqs", str(save_path))

                save_file = (
                    self.config_dirs[ind]
                    / f"{self.io_cfg['save_file_names']['targ_sweep']}_config_drone_{self.timestamp}{f'_{timestamp}' if not timestamp == self.timestamp else ''}.yaml"
                )
                drone_cfg = io.save_config(save_file, drone_cfg, self.save_cfg)
            return found_freqs
        ...
        window = autils.get_drone_args(
            self, com_to, ["tune_placement", "grad", "trim", "window"]
        )
        trim_savgol_window = autils.get_drone_args(
            self, com_to, ["tune_placement", "grad", "trim", "savgol_window"]
        )
        diff_savgol_window = autils.get_drone_args(
            self, com_to, ["tune_placement", "grad", "diff", "savgol_window"]
        )
        trim_savgol_order = autils.get_drone_args(
            self, com_to, ["tune_placement", "grad", "trim", "savgol_order"]
        )
        diff_savgol_order = autils.get_drone_args(
            self, com_to, ["tune_placement", "grad", "diff", "savgol_order"]
        )
        ...
        for key, value in kwargs.items():
            if key == "sweep_steps":
                ...
                autils.set_drone_args(
                    self, com_to, ["tune_placement", "method"], method
                )
            elif key == "window":
                window = autils.parse_args(self, com_to, value)
                autils.set_drone_args(
                    self, com_to, ["tune_placement", "grad", "trim", "window"], method
                )
                ...
                autils.set_drone_args(
                    self,
                    com_to,
                    ["tune_placement", "grad", "trim", "savgol_window"],
                    method,
                )
                ...
                autils.set_drone_args(
                    self,
                    com_to,
                    ["tune_placement", "grad", "diff", "savgol_window"],
                    method,
                )
                ...
                autils.set_drone_args(
                    self,
                    com_to,
                    ["tune_placement", "grad", "trim", "savgol_order"],
                    method,
                )
                ...
                autils.set_drone_args(
                    self,
                    com_to,
                    ["tune_placement", "grad", "diff", "savgol_order"],
                    method,
                )
            elif key == "write_targ_comb":
                write_targ_comb = value
        ...
        for i, com in enumerate(com_to):
            ...
            targ_file = io.get_most_recent_file(
                self.targ_dirs[ind],
                f"{self.io_cfg['save_file_names']['targ_sweep'][0]}*",
                time_past=24 * 3600,
            )
        ...
        if len(to_sweep) != 0:
            ...
            for i in range(len(com_to)):
                if targ_files[i] is None:
                    targ_files[i] = new_files.pop(
                        0
                    )  # Add old vna files into list of new vna files sorted by drone com_to
                if timestamps[i] == -1:
                    timestamps[i] = self.timestamp
        ...
        methods = {"min": _place_min, "grad": _place_grad}

        tone_freqs = [None] * len(com_to)
        for method_name, method_func in methods.items():
            method_inds = [
                i for i, com_method in enumerate(method) if com_method == method_name
            ]
            method_com_to = np.array(com_to)[method_inds].tolist()
            if len(method_com_to) > 0:
                tone_freq = method_func(method_com_to, method_inds)
                for ind, freq in zip(method_inds, tone_freq):
                    tone_freqs[ind] = freq
        if write_targ_comb:
            log.log(
                "INFO",
                "Writing target comb using tuned tone frequencies for drones %s!",
                com_to,
            )
            ...
            ccatkidlib_com_to, ccatkidlib_inds = (
                [],
                [],
            )  # Drones with ccatkidlib tuning method specified
            ...
            if len(untuned_com_to) > 0:
                log.log(
                    "WARNING",
                    "Invalid tuning method specified for drones %s! Not writing target comb for these drones.",
                    untuned_com_to,
                )
            ...
            if len(ccatkidlib_com_to) > 0:
                kwargs["com_to"], kwargs["tone_freqs"] = (
                    ccatkidlib_com_to,
                    [tone_freqs[ind] for ind in ccatkidlib_inds],
                )
                for k, v in kwargs.items():
                    if isinstance(v, Iterable) and len(v) == len(com_to):
                        kwargs[k] = [v[ind] for ind in ccatkidlib_inds]
                rtn = self.write_config_comb(**kwargs)

            log.log(
                "INFO",
                "Finished writing target combs for drones %s!",
                primecam_readout_com_to + ccatkidlib_com_to,
            )
    ...
    def _save_sweep(self, com_to, sweep, dirs):
        ...
        for com in com_to:
            ...
            try:
                ...
                file = io.get_most_recent_file(
                    self.tmp_data_dir,
                    file_identifier=self.io_cfg["board_file_names"][f"{sweep}"]["s21"]
                    + drone_id,
                    time_past=time.time() - float(self.timestamp),
                )

                # Save the data with name specified in system config file
                if self.save_data:
                    data = io.get_array(file, save_path, action="mv", load=False)
                    if data is None:
                        log.log(
                            "WARNING",
                            f"Failed to find {sweep} sweep file for drone {com} in {self.tmp_data_dir}! Fetching file directly from board instead.",
                        )
                ...
                with io.get_connection(bip, ssh_key) as c:
                    ...
                    file = io.get_most_recent_file_board(
                        c,
                        path,
                        file_identifier=self.io_cfg["board_file_names"][f"{sweep}"][
                            "s21"
                        ],
                        time_past=time.time() - float(self.timestamp),
                    )
                    ...
                    # Save the data with name specified in system config file
                    if self.save_data:
                        data = io.get_array_board(
                            c, bip, ssh_key, file, save_path, load=False
                        )
                    else:
                        data = file if io.path_exists(c, file) else None
            ...
            log.log("DEBUG", f"Successfully copied {sweep} file from drone {com}!")
            paths.append(save_path)

        # Save ext config
        self.ext_cfg = io.save_config(
            self.log_dir
            / f"{self.io_cfg['save_file_names'][f'{sweep}_sweep']}_config_ext_{self.timestamp}.yaml",
            self.ext_cfg,
            self.save_cfg,
        )
    ...
    def _save_resonators(self, com_to, timestamps, sweep):
        ...
        for com, timestamp in zip(com_to, timestamps):
            ...
            try:
                drone_id = f"_{bid}_{drid}*"
                res_file = io.get_most_recent_file(
                    self.tmp_data_dir,
                    file_identifier=self.io_cfg["board_file_names"][f"{sweep}"]["res"]
                    + drone_id,
                    time_past=time.time() - float(self.timestamp) + 30,
                )
                res_path = io.get_array(
                    res_file, res_dir / fname, action="mv", load=False
                )
                if res_path is None:
                    raise FileNotFoundError
            except FileNotFoundError:
                ...
                with io.get_connection(bip, ssh_key) as c:
                    ...
                    res_file = io.get_most_recent_file_board(
                        c,
                        path,
                        file_identifier=self.io_cfg["board_file_names"][f"{sweep}"][
                            "res"
                    ...
                    res_path = io.get_array_board(
                        c, bip, ssh_key, res_file, res_dir / fname, load=False
                    )
            ...
            log.log("INFO", f"Found {found_num} detectors for drone {com}!")

            io.edit_config(self.drone_cfg[ind], "found_num_detectors", found_num)
            io.edit_config(self.drone_cfg[ind], "found_detector_freqs", res_path)

            save_file = (
                self.config_dirs[ind]
                / f"{self.io_cfg['save_file_names'][f'{sweep}_sweep']}_config_drone_{self.timestamp}{f'_{timestamp}' if not timestamp == self.timestamp else ''}.yaml"
            )
            save_files.append(save_file)
            self.drone_cfg[ind] = io.save_config(
                save_file, self.drone_cfg[ind], self.save_cfg
            )
    ...
    def _write_custom_comb(self, com, rescale_power=True, gen_attempts=5, **kwargs):
        ...
        def _check_comb(key, value):
            ...
            if key == "tone_freqs":
                ...
                if (
                    np.max(np.abs(value - self.drone_cfg[ind]["tones"]["NCLO"] * 1e6))
                    > self.drone_cfg[ind]["tones"]["full_bandwidth"] * 1e6 / 2
                ):
                    log.log(
                        "WARNING",
                        f"{value} contains frequencies outside of the RFSoC bandwidth. Not writing {key} custom comb!",
                    )
                    return None
        ...
        def _reset_comb(ip, ssh_key, comb_dict):
            for key, value in comb_dict.items():
                io.save_array_board(
                    ...
                    ssh_key,
                    value["path"],
                    _arr_to_list(value["comb"]),
                    self.tmp_dir,
                )

            # Return unmodified comb
            return (
                comb_dict["tone_freqs"]["comb"],
                comb_dict["tone_powers"]["comb"],
                comb_dict["tone_phis"]["comb"],
            )
        ...
        def _write_new_comb(ip, ssh_key, comb_dict, key, comb):
            ...
            io.save_array_board(
                ip, ssh_key, comb_dict[key]["path"], _arr_to_list(comb), self.tmp_dir
            )
        ...
        if connection is None:
            connection = io.get_connection(bip, ssh_key)
        ...
        with connection as c:
            ...
            if freq_comb[0] < 0:
                freq_comb = (
                    np.array(freq_comb) + self.drone_cfg[ind]["tones"]["NCLO"] * 1e6
                )

            freq_path = io.get_most_recent_file_board(
                c,
                custom_comb_dir,
                file_identifier=self.io_cfg["board_file_names"]["targ"]["comb_freq"],
                time_past=np.inf,
            )
            amp_path = io.get_most_recent_file_board(
                c,
                custom_comb_dir,
                file_identifier=self.io_cfg["board_file_names"]["targ"]["comb_amp"],
                time_past=np.inf,
            )
            phi_path = io.get_most_recent_file_board(
                c,
                custom_comb_dir,
                file_identifier=self.io_cfg["board_file_names"]["targ"]["comb_phi"],
                time_past=np.inf,
            )
            ...
            for key, value in kwargs.items():
                ...
                try:
                    ...
                    if not io.path_exists(c, path):
                        log.log(
                            "ERROR",
                            f"Could not find {key} custom comb file at path {path}.",
                        )
                        raise FileNotFoundError
                ...
                if isinstance(value, (str, Path)):
                    ...
                    if not value.exists():
                        log.log(
                            "WARNING",
                            f"{value} is not a valid {key} custom comb file path for drone {com}!",
                        )
                        comb_dict = _write_new_comb(
                            bip, ssh_key, comb_dict, key, comb_dict[key]["comb"]
                        )
                ...
                # If not file path, assume value is a number or an array of numbers
                    ...
                    try:
                        ...
                        if len(value) == 0:
                            log.log(
                                "WARNING",
                                f"'{value}' is an empty array, not writing to {key} custom comb file for drone {com}!",
                            )
                            comb_dict = _write_new_comb(
                                bip, ssh_key, comb_dict, key, comb_dict[key]["comb"]
                            )
                        ...
                        if np.issubdtype(
                            type(value), (np.integer, np.floating)
                        ):  # Assume a number is passed and use the same number for all tones
                            comb_dict[key]["num_tones"] = float(
                                value
                            )  # Store the number in num_tones as a float for processing later since the number of tones in the comb may be unknown
                            continue
                        else:
                            # Invalid value passed; ignore and move on to next comb
                            log.log(
                                "WARNING",
                                f"{value} is not a valid file path, array, or number! Not writing {key} custom comb file for drone {com}.",
                            )
                            comb_dict = _write_new_comb(
                                bip, ssh_key, comb_dict, key, comb_dict[key]["comb"]
                            )
            ...
            # -----------------------------------------------------------------------------------------------------
            for key, value in comb_dict.items():
                if value["num_tones"] is None:
                    comb_dict[key]["num_tones"] = len(value["comb"])
            ...
            for key, value in comb_dict.items():
                ...
                if type(comb_val) is float:
                    ...
                    if comb_val is not None:
                        comb_dict = _write_new_comb(
                            bip, ssh_key, comb_dict, key, comb_val
                        )
                    else:
                        # Resetting back to current comb
                        log.log(
            ...
            tone_freqs_bb = (
                np.array(tone_freqs) - self.drone_cfg[ind]["tones"]["NCLO"] * 1e6
            )

            # Determine if tone powers need to be generated
            # ---------------------------------------------
            tone_powers = (
                np.ones(tone_num) * max_power / np.sqrt(tone_num)
                if gen_amps
                else comb_dict["tone_powers"]["new_comb"]
            )
            ...
            if gen_phis:
                ...
                io.save_array_board(
                    bip, ssh_key, phi_path, _arr_to_list(tone_phis), self.tmp_dir
                )
            else:
            ...
            if best_peak > max_power and rescale_power:
                log.log(
                    "WARNING",
                    f"The custom comb has points(s) with output power {best_peak} which exceeds the maximum DAC power of {max_power}! Rescaling to lower tone power.",
                )
                tone_powers *= max_power / best_peak

            if gen_amps or rescale_power:
                io.save_array_board(
                    bip, ssh_key, amp_path, _arr_to_list(tone_powers), self.tmp_dir
                )
            ...
            # Frequency comb
            io.edit_config(
                self.drone_cfg[ind], ["tones", "tone_freqs"], _arr_to_list(tone_freqs)
            )

            # Amplitude comb
            io.edit_config(
                self.drone_cfg[ind], ["tones", "tone_powers"], _arr_to_list(tone_powers)
            )

            # Phase comb
            io.edit_config(
                self.drone_cfg[ind], ["tones", "tone_phis"], _arr_to_list(tone_phis)
            )
    ...
    def _get_curr_comb(self, com, **kwargs):
        ...
        try:
            ...
            files = [
                io.get_most_recent_file(
                    self.tmp_data_dir,
                    file_identifier=[
                        self.io_cfg["board_file_names"]["vna"][name] + drone_id,
                        self.io_cfg["board_file_names"]["targ"][name] + drone_id,
                    ],
                    time_past=np.inf,
                )
                for name in names
            ...
            combs = [
                io.get_array(file, self.tmp_dir, action="cp", timestamp=True, load=True)
                for file in files
            ]
        except FileNotFoundError:
            # Parse com information
            ...
            with connection as c:
                # Get most recent comb file paths
                files = [
                    io.get_most_recent_file_board(
                        c,
                        comb_dir,
                        file_identifier=self.io_cfg["board_file_names"]["targ"][name],
                        time_past=np.inf,
                    )
                    for name in names
                ]
                ...
                combs = [
                    io.get_array_board(
                        c, bip, ssh_key, file, self.tmp_dir, timestamp=True, load=True
                    )
                    for file in files
                ]
    ...
    def _save_curr_comb(self, com, name, **kwargs):
        ...
        if from_board:
            ...
            with connection as c:
                ...
                amp_path = io.get_array_board(
                    c, bip, ssh_key, amp_file, amp_name, load=False, timestamp=timestamp
                )
                phi_path = io.get_array_board(
                    c, bip, ssh_key, phi_file, phi_name, load=False, timestamp=timestamp
                )
        else:
            # Copy most recent comb files
            freq_path = io.get_array(
                freq_file, freq_name, action="cp", load=False, timestamp=timestamp
            )
            amp_path = io.get_array(
                amp_file, amp_name, action="cp", load=False, timestamp=timestamp
            )
            phi_path = io.get_array(
                phi_file, phi_name, action="cp", load=False, timestamp=timestamp
            )

        io.edit_config(self.drone_cfg[ind], ["tones", "tone_freqs"], freq_path)
        io.edit_config(self.drone_cfg[ind], ["tones", "tone_powers"], amp_path)
        io.edit_config(self.drone_cfg[ind], ["tones", "tone_phis"], phi_path)

        if freq_path is not None:
            io.edit_config(
                self.drone_cfg[ind],
                ["tones", "num_tones"],
                len(np.load(freq_path, mmap_mode="r")),
            )

        # Save drone config
        self.drone_cfg[ind] = io.save_config(
            self.config_dirs[ind] / f"{name}_config_drone_{self.timestamp}.yaml",
            self.drone_cfg[ind],
            self.save_cfg,
        )
    ...
    def _run_parallel(
        ...
        func: Callable,
        *args,
        parallel_boards: int | None = None,
        parallel_drones: int | None = None,
        **kwargs,
    ) -> list[any]:
        ...
        if parallel_boards is None:
            parallel_boards = self.parallel_boards
        if parallel_drones is None:
            parallel_drones = self.parallel_drones

        if parallel_boards == -1:
            parallel_boards = len(boards)
        ...
        high_ind = (
            0  # Highest index of com_arrs containing a com_to array with open space
        )
        ...
        com_arrs = [
            com_arr for com_arr in com_arrs if com_arr
        ]  # Remove empty com_to arrays
