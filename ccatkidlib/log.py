...
Level = Literal[
    "PCS", "DEBUG", "TIMER", "INFO", "HEADER", "WARNING", "ERROR", "CRITICAL"
]
...
def setup_logging(
    ...
    def _addLevel(name: str, num: int) -> None:
        ...
        if not hasattr(logging, name):
            ...
            setattr(
                logging, name, num
            )  # Add new attribute to the logging class corresponding to custom logging level

            # Add new methods to relevant loggging classes
            setattr(
                logging.getLoggerClass(),
                method_name,
                partialmethod(logging.getLoggerClass().log, num),
            )
    ...
    # -------------------------
    custom_levels = [
        ["HEADER", int((logging.INFO + logging.WARNING) / 2)],
    ...
    if not any(isinstance(handler, RotatingFileHandler) for handler in logger.handlers):
        file_log = RotatingFileHandler(
            log_path, mode="a", maxBytes=max_file_size * 1024**2, backupCount=1
        )
    ...
    if not any(
        isinstance(handler, tqdm_logging._TqdmLoggingHandler)
    ...
    logger.setLevel(
        min(file_level, terminal_level)
    )  # Set logger level to the minimum of file and terminal levels
...
def log(level: Level, msg: str, *args, name: str = __name__) -> None:
    ...
    try:
        ...
        logger.log(log_level, msg, *args) if not len(args) == 0 else logger.log(
            log_level, msg
        )
...
def wait(t: float, desc: str = "", interval: float = 0.01) -> None:
    ...
    with tqdm(
        total=t, colour="BLUE", desc=f"{Style.style_level('WAIT')} {desc}"
    ) as pbar:
        while time_diff < t:
...
def header(method: Callable) -> Any:
    ...
    @wraps(
        method
    )  # Needed for help() calls on ``method`` to print help string of ``method`` instead of help string of header
    def _wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            log(
                "ERROR",
                "Method %s failed to execute with error:\n%s",
                fmt,
...
def method_timer(method: Callable) -> Any:
    ...
    def _wrapper(self, *args, **kwargs):
        ...
        log(
            "TIMER", f"Method {Style.style_name(name)} executed in {time_diff} seconds."
        )
        return rtn
