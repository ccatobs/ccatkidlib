...
def create_tree(
    com_to: list[str], curr_date: str, sess_id: int, data_dir: str
...
def create_dir(dir: str) -> None:
    ...
    try:
        ...
        if not dir.exists():
            ...
            log.log(
                "DEBUG",
                "The directory '%s' already exists! Directory was not overwritten.",
                dir,
            )
    except FileNotFoundError:
        log.log(
            "ERROR",
            "The directory '%s' could not be created! Ensure that the file path is valid!",
            dir,
        )
        raise FileNotFoundError(
            f"The directory '{dir}' could not be created! Ensure that the file path is valid!"
        )
    except PermissionError:
        log.log(
            "ERROR",
            "The directory '%s' could not be created! Ensure that the parent directory has suitable write permissions!",
            dir,
        )
        raise PermissionError(
            f"The directory '{dir}' could not be created! Ensure that the parent directory has suitable write permissions!"
        )
...
def load_config(cfg_path: str) -> dict | list[dict]:
    ...
    if not cfg_path.exists():
        raise FileNotFoundError(
            f"Could not find config file: '{cfg_path}'"
...
def edit_config(cfg: dict, key: str, value: Any, append: bool = False) -> bool:
    ...
    if done:  # If matching key was updated
        ...
        append
    ):  # If key was not found and append=True, add key value pair to dictionary
        ...
        log.log(
            "DEBUG",
            f'Failed to update key "{key}" with value "{value}" in config file!',
        )
    return done
...
def get_most_recent_file(
    dir: str,
    file_identifier: str | list[str] = "*",
    time_past: float = 60 * 60,
    time_ref: float | None = None,
    ccatkidlib_file: bool = False,
) -> str:
    """
    ...
    try:
        ...
        while max_ind >= min_ind:
            ...
            if shifted_time < 0:  # If time difference is negative, then file is too new
                ...
                shifted_time > 0 and not (max_ind == min_ind)
            ):  # If time difference is positive, then file is a candidate but there could be more recent files
                max_ind = curr_ind
        ...
        log.log(
            "DEBUG",
            f"Failed to fetch most recent file in '{dir}' with identifier '{file_identifier}' with Exception:\n{e}",
        )
...
def get_timestamp(path: str) -> int:
    ...
    try:  # Check that the passed path is a string or Path object
        ...
        for i in [
            ...
            -2,
            0,
        ]:  # Timestamp should only be at the 0, -1, or -2 index of the file part list
            try:
                tstamp = int(parts[i])  # Try casting part of file to int
                if tstamp > 1.7e9:
                    return tstamp  # Check that integer is a valid timestamp
            except:
                pass
        raise ValueError(
            f"The file {file} has no valid timestamp."
        )  # Raise ValueError if timestamp could not be determined
    except (
        Exception
    ) as e:  # If exception is thrown, return -1 to represent invalid path
        log.log(
            "ERROR",
            "Failed to determine timestamp of file %s with Exception %s",
            path,
            e,
        )
        return -1
...
def get_array(
    ...
    if Path(src_path).exists():  # Ensure that path to numpy array exists on RFSoC board
        ...
        if timestamp:
            file = "_".join(file.split("_")[:-1])  # Trim timestamp off file if desired

        if Path(dest_path).suffix == "":
            dest_path += f"/{file}{ext}"  # If local path is a directory, use same file name as on RFSoC board
...
def increment_file(
    dir: str, file_prefix: str, file_suffix: str, overwrite: bool = False
) -> tuple[str, int | None]:
    """
...
def combine_npy(
    files: list[str], num: int, com: str | None = None, fname_out: dict | None = None
) -> list[str]:
    """
    ...
    num_zipped = (
        int(num_zipped) if num_zipped == int(num_zipped) else int(num_zipped) + 1
    )

    fname = str(
        Path(files[0]).parent / "_".join(str(Path(files[0]).stem).split("_")[:-1])
    )
    fnames = []
    for i in range(num_zipped):
        ...
        fnames.append(name)
        loaded_files = {
            str(Path(file).stem): np.load(file, mmap_mode="r")
            for file in files[i * num : (i + 1) * num]
        }
...
def get_array_board(
    ...
    if path_exists(
        c, remote_path
    ):  # Ensure that path to numpy array exists on RFSoC board
        # Copy numpy file from board and load locally
        # -------------------------------------------
        # Get name of file to be copied
...
def save_array_board(
    ip: str, ssh_key: str, path: str, array: np.ndarray, tmp_dir: str
) -> str | None:
    """
...
def get_most_recent_file_board(
    c: Connection, dir: str, file_identifier: str = "*", time_past: float = 60 * 60
) -> str:
    """
    ...
    if not time_past == np.inf:
        ...
        time_past = (
            int(time_past) if time_past == int(time_past) else int(time_past) + 1
        )
    ...
    try:
        ...
        log.log(
            "WARNING",
            f"Failed to fetch most recent file in '{dir}' with identifier '{file_identifier}'",
        )
        return "invalid/path"
...
def path_exists(c: Connection, path: str) -> bool:
    ...
    return ast.literal_eval(
        c.run(cmd, hide="out", in_stream=False).stdout
    )  # Run command on RFSoC board and get command stdout
