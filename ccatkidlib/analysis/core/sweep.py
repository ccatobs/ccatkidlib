...
class Sweep(Data):
    ...
    def _plot(df, plot_opts, *args, **kwargs):
        ...
        sweep = df.hvplot.line(x=x_dim, y=y_dim, label="Data", **kwargs).relabel(
            group="Sweep"
        )
        ...
        tone = (
            df.filter(pl.col("tone")).hvplot.scatter(
                x=x_dim, y=y_dim, label="Tone", **kwargs
            )
        ).relabel(group="Sweep")
        plot = sweep * tone
        if "area_sample" in df.schema:
            if df.schema["area_sample"] == pl.List:
                df = df.explode("area_sample")
            area = (
                df.filter(pl.col("sample") == pl.col("area_sample"))
                .hvplot.area(x=x_dim, y=y_dim, label="Area Data", **kwargs)
                .relabel(group="Sweep")
            )
            plot *= area
    ...
    def plot(
        ...
        **kwargs,
        ...
        SweepFigure | tuple[SweepFigure, pl.DataFrame] | tuple[pl.DataFrame, str | None]
    ):
        r"""
        ...
        if df is None or by is None:
            ...
            df = df.with_columns(
                pl.when(pl.col(col_dict["sample"]) == tone_sample)
                .then(pl.lit(True))
        ...
        overlay_opts = opts.Overlay(
            title=title,
            xlabel=xlabel if xlabel is not None else col_dict["x"],
            ylabel=ylabel if ylabel is not None else col_dict["y"],
            fig_size=kwargs.pop("fig_size")
            if "fig_size" in kwargs
            else self.viz_cfg["static_plot"]["sweep"]["fig_size"],
        )
        curve_opts = opts.Curve(show_legend=False)
        scatter_opts = opts.Scatter(show_legend=False)
        area_opts = opts.Area(
            alpha=kwargs.pop("area_alpha")
            if "area_alpha" in kwargs
            else self.viz_cfg["static_plot"]["sweep"]["area_alpha"]
        )
        all_opts = [overlay_opts, curve_opts, area_opts]
        if plot_opts is not None:
            all_opts += plot_opts if isinstance(plot_opts, Iterable) else [plot_opts]
    ...
    def IQ_plot(
        ...
        **kwargs,
        ...
        if projection == "IQ":
            ...
            mask = [
                np.all(filt) for filt in zip(x_ticks != np.pi, x_ticks != np.pi / 2)
            ]
            ...
            # ----------------------------------------------------------------
            if max_IQ_sliver:
                area_df = ccat_df.get_properties(
                    self,
                    col_name="max_IQ_dist_sample",
                    include=include,
                    exclude=exclude,
                    strict=True,
                )
                if "max_IQ_dist_sample" in area_df.schema:
                    area_df = area_df.select(
                        pl.col("det"),
                        pl.concat_list(
                            [
                                pl.col("max_IQ_dist_sample"),
                                pl.col("max_IQ_dist_sample") - 1,
                            ]
                        ).alias("area_sample"),
                    )
        ...
        if "plot_opts" in kwargs:
            plot_opts += (
                extra_opts
                if isinstance(extra_opts := kwargs.pop("plot_opts"), Iterable)
                else [extra_opts]
            )
