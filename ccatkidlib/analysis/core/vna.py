...
class VNA(Sweep):
    ...
    def stitch_phase(
        self,
        threshold: float = 1.9 * np.pi,
        stitch_percent: float = 10.0,
        recalc: bool = False,
    ...
    def filter_det_f(
        ...
        phase = (
            self.stitch_phase().to_numpy().T[0]
            if stitch_phase
            else self.phase().to_numpy().T[0]
        )
