...
def circle_fit(
    ...
    @njit(cache=True)
    def _res_jac(
        A: float, D: float, theta: float, x: np.ndarray, y: np.ndarray
    ...
    shift_x, shift_y = 0, 0
    if center_mag < 5:
        shift_x, shift_y = 50 / center_mag, 50 / center_mag
    x += shift_x
    y += shift_y
    ...
    elif not bounds[0][0] < init_A < bounds[1][0]:
        left, right = bounds
...
def init_circle_fit(x, y):
    ...
    M = _moments(x, y)
    B_inv = np.array([[0, 0, 0, -0.5], [0, 1, 0, 0], [0, 0, 1, 0], [-0.5, 0, 0, 0]])
...
def phase_fit(
    ...
    def _guess_params(f, phase):
        ninety_phase_ind = np.argmin(
            np.abs(np.abs(phase) - np.pi / 2)
        )  # Get the index at which the phase is closest to 90 degrees

        f_0_guess = (
            f[zero_phase_ind_up] + f[zero_phase_ind_low]
        ) / 2  # Guess resonant frequency to be near zero phase
        f_0_min, f_0_max = (
            f[0],
            f[-1],
        )  # Bound resonant frequency to be within sampled frequencies
        ...
        Qr_guess = (
            f_0_guess / FWHM_guess if FWHM_guess != 0 else np.sqrt(Qr_min * Qr_max)
        )
    ...
    # Unwrap phase
    # ------------
    zero_phase_ind_up, zero_phase_ind_low = (
        np.argmin(phase[phase >= 0]),
        np.argmax(phase[phase <= 0]),
    )
