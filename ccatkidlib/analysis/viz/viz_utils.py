...
def cycle_cmap(
    cmap: str, num_colors: int, cmap_range: tuple[float, float] = (0, 1)
) -> hv.Cycle | None:
    ...
    if cmap in list(mpl.colormaps):
        ...
        colors = [
            mpl.colors.rgb2hex(c)
            for c in cmap(np.linspace(cmap_range[0], cmap_range[1], num_colors))
        ]
...
def save_fig(
    ...
    **kwargs,
    ...
    if save_fig is None:
        save_fig = viz_cfg["save"]["save_fig"]
    if figs_per_file is None:
        figs_per_file = viz_cfg["save"]["figs_per_file"]
    if overwrite is None:
        overwrite = viz_cfg["save"]["overwrite"]
    if save_dir is None:
        save_dir = Path(obj.fig_dir)
    if save_name is None:
        save_name = "tmp"
    if save_fmt is None:
        save_fmt = viz_cfg["save"]["fig_fmt"]
    ...
    if isinstance(plot, hv.HoloMap):
        ...
        for i, (key, fig) in enumerate(plot.items()):
            if kdims:  # Edit subfigure titles to include key dimensions
                ...
                fig.opts(
                    title=", ".join(
                        [f"{name}={value}" for name, value in zip(kdims, key)]
                    )
                )
        ...
        for i in range(len(plots)):
            plots[i] = (
                hv.Layout(plots[i])
                .opts(sublabel_format="", shared_axes=False)
                .cols(int(np.sqrt(figs_per_file)))
            )
    else:
        plots = [plot]

    save_path = io.increment_file(
        save_dir, f"{save_name}_{timestamp}_", f"_0.{save_fmt}", overwrite=overwrite
    )[0]
    save_path = "_".join(str(save_path.with_suffix("")).split("_")[:-1])

    for i, plot in enumerate(plots):
        hv.save(
            plot, f"{save_path}_{i}", fmt=save_fmt, backend="matplotlib", toolbar=False
        )
