...
def get_sess_dir(
    sess_id, data_dir: str = "**", date: str = "**", root_data_dir: str = "/"
) -> str:
...
def get_data_file(
    ...
    file_trees = [
        Path(data_dir) / date / sess_id / data_type / com_str,
    ...
    for tree in file_trees:
        ...
        if len(data_files) > 0:
            return data_files  # Return data files, return as list because multiple timestream files could be found
    return ["invalid/path"]
...
def get_config(path: str | pathlib.PosixPath, all_cfg: bool = False) -> list[str]:
    ...
    if not timestamp == -1:  # Make sure a valid data file is passed
        data_names = [
            ...
            "targ",
            "timestream",
        ]  # Directory names of main three data file types
        ind = -1
        ...
        for name in ["config", "rfsoc"]:
            ...
            if not ind == -1:
                parts[ind] = (
                    name  # If not the first iteration through loop, part of path that needs to be replaced is already known
                )
            else:
                for i, part in enumerate(
                    parts
                ):  # If first iteration through loop, find part in path that needs to be replaced
                    if any(
                        part == data_name for data_name in data_names
                    ):  # Check if part name matches any of the data_names
                        parts[i] = name
                        ind = i
            ...
            # ------------------------------------------------
            cfg_path = Path(*parts)
            io_cfg = io.get_most_recent_file(
                cfg_path.parent, "*io*.yaml", time_past=np.inf
            )  # Get io config file
            if io_cfg.exists():  # If io config exists, then config directory is correct
                if all_cfg:
                    cfg_path = cfg_path.parent  # If all_cfg, change to parent directory so that configs for all drones are found
                ...
                if not all_cfg:  # Find ext config if all_cfg is False (since it is not in the drone config directory)
                    ext_cfg = io.get_most_recent_file(
                        cfg_path.parent, f"*{timestamp}*.yaml", time_past=np.inf
                    )
                    cfgs += [ext_cfg]
...
def get_sweep(path: str | pathlib.PosixPath, **kwargs):
    ...
    if not timestamp == -1:  # Make sure a valid data file is passed
        ...
        for i, parts in enumerate(parts_list):
            ...
            sweeps[i] = io.get_most_recent_file(
                sweep_path,
                "*.npy",
                time_past=np.inf,
                time_ref=timestamp,
                ccatkidlib_file=True,
            )
...
def replace_root(path: str | pathlib.PosixPath, old_root: str, new_root: str):
    ...
    if Path(new_path).exists():
        ...
        log.log(
            "ERROR",
            "Could not find file %s with original root directory %s replaced with %s",
            new_path,
            old_root,
            new_root,
        )
